...
import { Inquiry, Brand, LeadStatus } from '@/types';
...
export default function EditLeadModal({ lead, onClose, onSaved }: Props) {
  ...
  const [isCustomAssignee, setIsCustomAssignee] = useState(!INITIAL_ASSIGNEES.includes(lead.assignee));
  const [customAssignee, setCustomAssignee] = useState(INITIAL_ASSIGNEES.includes(lead.assignee) ? '' : lead.assignee);
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-900">問い合わせ 編集</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              ...
              {(['escot', 'kakitsubata'] as Brand[]).map((b) => (
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-all ${
                    form.brand === b
                      ? b === 'escot' ? 'bg-blue-50 border-blue-300 text-[#1A2F4B]' : 'bg-purple-50 border-purple-300 text-[#5D3176]'
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={submitting} className="flex-1 py-2.5 rounded-lg text-sm font-medium text-white transition-all disabled:opacity-60" style={{ background: 'var(--color-primary)' }}>
              {submitting ? '更新中...' : '更新する'}
            </button>
          </div>
        </form>
