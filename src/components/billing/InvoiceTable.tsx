...
export default function InvoiceTable({ initialData }: Props) {
  const [data, setData] = useState<Invoice[]>(initialData);
  const { brand: activeBrand } = useBrandTheme();

  const toggleStatus = (id: string, currentStatus: InvoiceStatus) => {
    setData(prev => prev.map(item => {
      if (item.id === id) {
        let newStatus: InvoiceStatus = currentStatus;
        let sentDate = item.sentDate;
        ...
        return { ...item, status: newStatus, sentDate };
      }
      return item;
    }));
  };
                ...
                <td className="px-4 py-3 text-right">
                  {item.status !== '発送済み' ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 text-primary hover:bg-primary/20 transition-colors rounded-md text-xs font-semibold"
                    >
                      {item.status === '日程決め' ? (
                        <>作成完了にする</>
                      ) : (
                    ...
                    <button
                      onClick={() => toggleStatus(item.id, item.status)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors rounded-md text-xs font-semibold"
                    >
                      リセット
                    </button>
                  )}
