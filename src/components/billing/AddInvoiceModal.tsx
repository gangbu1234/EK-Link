...
import { X, ReceiptText, Plus } from 'lucide-react';
import { Brand, InvoiceStatus } from '@/types';
...
export default function AddInvoiceModal({ onClose, onSaved }: Props) {
  const [form, setForm] = useState({
    brand: 'escot' as Brand,
    studentId: '',
    studentName: '',
    assignee: '岡部',
    status: '日程決め' as InvoiceStatus,
  });
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-900">請求書 新規追加</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={form.studentName} onChange={e => setForm(f => ({ ...f, studentName: e.target.value }))} placeholder="例：山田 太郎" className={inputCls} />
          </div>
